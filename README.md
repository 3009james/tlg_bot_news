...
### Преднастроенный источник Groq

При старте бот автоматически создает источник `groq_llm` (тип `llm`) с OpenAI-compatible endpoint Groq.

Остается только добавить ключ:
1. В боте нажмите `API-ключи`.
2. Выберите источник `groq_llm`.
3. Нажмите `Добавить ключ`.
4. Введите:
: label, например `primary`
: secret_name: `Authorization`
: значение ключа Groq (формат `gsk_...`)
